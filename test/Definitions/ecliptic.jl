...
@testset "EME 2000" verbose = false begin
    ...
    @test v2as(R[1] * v, DCM_ICRF_TO_EME2000 * v) ≈ 0.0 atol = 1e-14 rtol = 1e-14
    ...
    # Check warning on axesid not being the true EME2000 ID
    @test_logs (:warn, "ECL2000 is aliasing an ID that is not the standard EME2000 ID" *
    " ($(AXESID_EME2000)).") add_axes_eme2000!(frames, ECL2000, ICRF)
...
@testset "ECL2000" verbose = false begin
    ...
    @test_throws ArgumentError add_axes_ecl2000!(frames, ECL2000, EME_TEST)
    ...
    add_axes_eme2000!(f1, EME2000, ICRF)
    add_axes_ecl2000!(f1, ECL2000, ICRF)

    add_axes_inertial!(f2, EME2000)
    add_axes_ecl2000!(f2, ECL2000, EME2000)
    ...
    # Check warning on axesid not being the true EME2000 ID
    @test_logs (:warn, "EME2000 is aliasing an ID that is not the standard ECL2000 ID" *
    " ($(AXESID_ECL2000)).") add_axes_ecl2000!(frames, EME2000, ICRF)
