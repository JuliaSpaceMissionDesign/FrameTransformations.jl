...
Add `axes` as a set of projected axes representing the True Equator and Equinox of Date (TOD) 
to `frames`. 
...
### See also 
See also [`add_axes_projected!`](@ref) and [`Orient.orient_rot3_icrf_to_tod`](@ref)
"""
@inline function add_axes_tod!(frames::FrameSystem, axes::AbstractFrameAxes, parent)
    return add_axes_tod!(frames, axes_name(axes), axes_id(axes), axes_alias(parent))
end
...
function add_axes_tod!(frames::FrameSystem, name::Symbol, axesid::Int, parentid::Int)
    ...
    if parentid != Orient.AXESID_ICRF
        throw(
            ArgumentError(
                "True Equator of Date (TOD) axes can only be defined " *
                "w.r.t. the ICRF (ID = $(Orient.AXESID_ICRF)).",
            )
        )
    end
