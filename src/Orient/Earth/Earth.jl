            ...
            for s in ctrig[jj][i]
                ...
                for (j, n) in enumerate(s.N)
                    if n != 0
                        push!(
                            argv,
                            Symbol("$(MAPPING[j])$(argmin(abs.(fad[MAPPING[j]] .- n)))"),
                        )
                    end
                end
...
function build_nutation_series(
    fname::Symbol, iau_model::Symbol, ψseries::AbstractVector{N}, ϵseries::AbstractVector{N}
    ...
    for i in 1:nblocks
        for j in 1:2 # Adds ψ and ϵ contributions 
            dct[(j, i)] = Vector{Int}() # initialises dictionary keys 
