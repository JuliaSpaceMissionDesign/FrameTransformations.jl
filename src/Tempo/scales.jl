...
"""
Basic.register!(s::TimeSystem{T}, ts::TimeScaleNode{T}) where T = add_vertex!(s.scales, ts)
...
@inline function apply_offsets(::TimeSystem{N}, sec::N, ::S, ::S) where {N<:Number, S<:AbstractTimeScale}
    return sec
end
