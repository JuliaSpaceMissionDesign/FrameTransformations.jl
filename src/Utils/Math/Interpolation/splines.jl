...
@inbounds function InterpCubicSplines(
    x::Vector{N}, y::Matrix{N}, type::Symbol=:Natural
) where {N}
    n = length(x)
...
@inbounds function CubicSplines(x::Vector{N}, y::Matrix{N}, type::Symbol=:Natural) where {N}
    n = length(x)
    n < 4 && throw(ArgumentError("At least four points are needed."))
    row, col = size(y)
    col != n && throw(ArgumentError("`x` and `y` must have the same length."))
...
@inbounds function _create_splines(::Val{:NotAKnot}, x::Vector{N}, y::Vector{N}) where {N}
    ...
    @views begin
        ...
        dl[1:(end - 1)] .= dx[2:end]
        b[2:(end - 1)] .=
            3 * (dx[2:end] .* slope[1:(end - 1)] .+ dx[1:(end - 1)] .* slope[2:end])

        # Not-a-knot boundary conditions
        d[1] = dx[2]
        du[1] = x[3] - x[1]
        δ = x[3] - x[1]
        b[1] = ((dx[1] + 2δ) * dx[2] * slope[1] + dx[1]^2 * slope[2]) / δ
        d[end] = dx[end - 1]
        dl[end] = x[end] - x[end - 2]
        δ = x[end] - x[end - 2]
        b[end] = (
            (dx[end]^2 * slope[end - 1] + (2δ + dx[end]) * dx[end - 1] * slope[end]) / δ
        )
...
@inbounds function _create_splines(::Val{:Quadratic}, x::Vector{N}, y::Vector{N}) where {N}
    ...
    @views begin
        ...
        dl[end] = dx[end - 1]
        b[end] =
            3 * dx[end - 1] * slope[end - 1] + 3 * dx[end] * slope[end - 1] -
            3 * dx[end] * slope[end - 2]
...
function interpolate(cs::InterpCubicSplines{N,S,T}, x) where {N,S,T}
    ...
    return SVector{S}(
        cs.c[1, j - 1, i] +
        dx * (cs.c[2, j - 1, i] + dx * (cs.c[3, j - 1, i] + dx * cs.c[4, j - 1, i])) for
        i in 1:S
    )
end
